                    ...
                    <div class="card-body" style="overflow:scroll;">
                        <table id="example1" class="table table-bordered table-striped">
                            ...
                            @foreach($daysOfWeek as $dayWithoutPrefix => $dayName)
                                            ...
                                            <td>                
                                                @for ($position = 1; $position <= 4; $position++)
                                                    @php
                                                        ...
                                                        $mealId = $position; 
                                                        $data = isset($structuredData[$week][$dayWithoutPrefix][$mealType][1]) 
                                                                ? $structuredData[$week][$dayWithoutPrefix][$mealType][1] 
                                                                : null;                           
                                                    @endphp    
                                                    <div> 
                                                        <center>
                                        ...
                                        <td>
                                            @for ($position = 5; $position <= 8; $position++)
                                                @php
                                                    ...
                                                    $data = isset($structuredData[$week][$dayWithoutPrefix][$mealType][1][$mealId]) 
                                                            ? $structuredData[$week][$dayWithoutPrefix][$mealType][1][$mealId] 
                                                            : null;
                                                @endphp    
                                                <div> 
                                                    <center>
                                ...
                                <tr class="accordion-content {{ strtolower($dayName) }}">
                                    <td class="meal-header" style="background-color: yellow">MEAL 4</td>
                                    <td class="meal-header" style="background-color: yellow"></td>
                                    <td class="meal-header" style="background-color: #09d921;">MEAL 4</td>
                                    <td class="meal-header" style="background-color: #09d921;"></td>
                                </tr>
                                <tr class="accordion-content {{ strtolower($dayName) }}">
                                    <!-- WEEK1 & WEEK2 -->
                                    @foreach([1, 2] as $week)
                                        ...
                                        @else
                                            <td>
                                                @for ($position = 13; $position <= 16; $position++)
                                                    @php
                                                        // Meal ID should remain the same across both weeks, starting from 13 to 16
                                                        $mealId = $position; 
                                                        // Fetch data for the current week and meal ID
                                                        $data = isset($structuredData[$week][$dayWithoutPrefix][$mealType][1][$mealId]) 
                                                                ? $structuredData[$week][$dayWithoutPrefix][$mealType][1][$mealId] 
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error) {
                    ...
                    content += '<input type="hidden" name="meal_id" value="1">'; 
