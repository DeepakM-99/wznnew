                                            ...
                                            <td>                
                                                @for ($position = 1; $position <= 4; $position++)
                                                    @php
                                                        ...
                                                        $mealId = $position; 
                                                        $data = isset($structuredData[$week][$dayWithoutPrefix][$mealType][3]) 
                                                                ? $structuredData[$week][$dayWithoutPrefix][$mealType][3] 
                                                                : null;                           
                                                    @endphp    
                                                    <div> 
                                                        <center>
                                        ...
                                        <td>
                                            @for ($position = 5; $position <= 8; $position++)
                                                @php
                                                    ...
                                                    $data = isset($structuredData[$week][$dayWithoutPrefix][$mealType][3][$mealId]) 
                                                            ? $structuredData[$week][$dayWithoutPrefix][$mealType][3][$mealId] 
                                                            : null;
                                                @endphp    
                                                <div> 
                                                    <center>
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.error) {
                    ...
                    content += '<input type="hidden" name="meal_id" value="3">'; // Set meal_id to 3
                    content += '<input type="hidden" name="position" value="' + position + '">'; // Set the position from data-meal_id
                    content += '<input type="hidden" name="meal_type" value="' + button.data('meal-type') + '">'; // Add meal_type to the form
                    content += '</form>';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Update the position and meal type with the saved menu item
                    var position = response.position;
                    var menuItem = response.menuItem;
                    var mealType = response.mealType; // Ensure mealType is part of the response
