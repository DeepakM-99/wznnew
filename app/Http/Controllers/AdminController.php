...
class AdminController extends BaseController
{
    ...
    public function food_menu(Request $request)
    {
    	...
    	return view('admin.food_menu', compact('data', 'category', 'inventory'));
    }
	...
	public function freezeMembership(Request $request)
	{
		// Fetch the current end_date from the form
		$frozenDays = $request->input('frozen_days');
		$currentEndDate = Carbon::parse($request->input('end_date'));

		// Calculate the new end_date by adding frozen days
		$newEndDate = $currentEndDate->addDays($frozenDays);

		// Update the database (assume you have a model `Membership`)
		DB::table('user_order')->where('order_id', $request->order_id)  // Assuming user_id is passed
			->update(['end_date' => $newEndDate]);

		// Find the next order for the user and update its start date
		$nextOrder = DB::table('user_order')
        ->where('user_id', function($query) use ($request) {
            $query->select('user_id')
                ->from('user_order')
                ->where('order_id', $request->order_id);
        })
        ->where('start_date', '>', Carbon::now()) // Ensure this is the upcoming order
        ->orderBy('start_date', 'asc')
        ->first();

		if ($nextOrder) {
			// Update the upcoming order's start date
			DB::table('user_order')
				->where('order_id', $nextOrder->order_id)
				->update([
					'start_date' => $request->upcoming_start_date,
					'end_date' => $request->upcoming_end_date
				]);
		}

		// Return response
		return redirect()->back()->with('message', 'Membership freeze applied successfully. New end date: ' . $newEndDate->toDateString());
	}

	public function getUpcomingMembershipStartDate($orderId)
	{
		// Find the current order based on the provided order ID
		$currentOrder = DB::table('user_order')->where('order_id', '=', $orderId)->first();
		if (!$currentOrder) {
			return response()->json(['error' => 'Order not found'], 404);
		}

		// Assuming you have a relationship to fetch the next order or membership
		$nextOrder =  DB::table('user_order')->where('user_id', $currentOrder->user_id)
			->where('start_date', '>', Carbon::now()) // Assuming 'start_date' is a column in your orders table
			->orderBy('start_date', 'asc')
			->first();

		if ($nextOrder) {
			// Return the next start date if it exists
			return response()->json(['next_start_date' => $nextOrder->start_date, 'next_end_date' => $nextOrder->end_date]);
		} else {
			// If no upcoming order, return a null or appropriate message
			return response()->json(['next_start_date' => null]);
		}
	}
	...
	public function pauseMembership(Request $request)
	{
		...
		$pauseStartDate = $request->input('pause_start_date'); // Date when the pause starts
		$pauseRestartDate = $request->input('pause_restart_date'); // Optional restart date
		...
		// Get current end_date from the order
		$orderEndDate = new Carbon($order->end_date);

		// Initialize variables
		$pauseStart = new Carbon($pauseStartDate);
		$pauseRestart = $pauseRestartDate ? new Carbon($pauseRestartDate) : null;

		// Calculate remaining days excluding Fridays
		$remainingDays = 0;
		$currentDate = $pauseStart->copy();

		while ($currentDate <= $orderEndDate) {
		...
		// If $pauseRestartDate is not provided, only update pause_day_count
		if ($pauseRestart === null) {
			// Update the order with the new pause_day_count only
			DB::table('user_order')
				->where('order_id', $orderId)
				->update([
					'is_paused' => 1,
					'restart_date' => $pauseRestartDate,
					'pause_day_count' => $remainingDays // Use the calculated remaining days
				]);

			// Return a response
			return redirect()->route('admin.order-list')->with('success', 'Membership paused successfully!');
		}

		// If a pause restart date is provided, recalculate end_date
		$newEndDate = $orderEndDate->copy();
		$daysAdded = 0;

		while ($daysAdded < $remainingDays) {
			$newEndDate->addDay();
			if ($newEndDate->dayOfWeek !== Carbon::FRIDAY) {
				$daysAdded++;
			}
		}

		// Recalculate new end date from the restart date if provided
		$remainingDaysFromRestart = 0;
		$currentDate = $pauseRestart->copy();

		// Ensure the restart date is not before the current end date
		if ($currentDate < $orderEndDate) {
			$currentDate = $orderEndDate->copy();
		}

		// Recalculate remaining days from the restart date
		while ($currentDate <= $newEndDate) {
			if ($currentDate->dayOfWeek !== Carbon::FRIDAY) {
				$remainingDaysFromRestart++;
			}
			$currentDate->addDay();
		}

		// Recalculate new end date from the restart date, skipping Fridays
		$newEndDate = $pauseRestart->copy();
		$daysAdded = 0;

		while ($daysAdded < $remainingDaysFromRestart) {
			$newEndDate->addDay();
			if ($newEndDate->dayOfWeek !== Carbon::FRIDAY) {
				$daysAdded++;
			}
		...
		// Update the order with the new end_date and pause_day_count
		DB::table('user_order')
			->where('order_id', $orderId)
			->update([
				'restart_date' => $pauseRestartDate,
				'pause_day_count' => $remainingDays, // Use the calculated remaining days
				'end_date' => $newEndDate->format('Y-m-d') // Update with the new end_date
			]);
	...
	public function getMeals(Request $request)
	{
		...
		$day = $request->input('day');
		$week = $request->input('week');
		$mealType = $request->input('meal_type'); // Assuming this is from meal_plan table
		$userPlanId = $request->input('user_plan_id');

		// Build the query
		$query = DB::table('generated_plans')
					->join('meal_plan', 'generated_plans.meal_id', '=', 'meal_plan.meal_id')
					->join('food_menu', 'generated_plans.menu_id', '=', 'food_menu.menu_id')
					->select('generated_plans.*', 'meal_plan.*', 'food_menu.*')
					->where('generated_plans.is_active', 1); // Always filter by active status

		// Apply filters only if values are present
		if ($mealId) {
			$query->where('generated_plans.meal_id', $mealId);
		}
		if ($day) {
			$query->where('generated_plans.day', $day);
		}
		if ($week) {
			$query->where('generated_plans.week', $week);
		}
		if ($menuId) {
			$query->where('generated_plans.menu_id', '!=', $menuId);
		}
		if ($mealType) {
			$query->where('generated_plans.meal_type', $mealType); // Assuming meal_type exists in meal_plan
		}
    ...
    {
        // Fetch the meal to update
		$meal_id = $request->input('meal_id');
		$user_plan_id = $request->input('user_plan_id');
		$menu_id = $request->input('menu_id');
        $meal = DB::table('user_plan')->where('user_plan_id', $user_plan_id)
                ->first();

		// Check if a matching meal is found
		if ($meal) {
			// Update the meal with the new menu_id
			DB::table('user_plan')
			...
			return redirect()->back()->with('message', 'Meal not found or user plan mismatch.');
		}
    }
